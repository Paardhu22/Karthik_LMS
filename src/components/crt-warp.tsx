...
export default function CRTWarp({
  ...
  useEffect(() => {
    ...
    try {
      ...
      // No WebGL context (old GPU, headless browser, blocked by policy). Bail out
      // quietly so the static CSS background behind this canvas shows through.
      console.warn('CRTWarp: WebGL unavailable, using static background.', error);
      geometry.dispose();
      material.dispose();
